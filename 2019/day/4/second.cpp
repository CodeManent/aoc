...
bool IsValidPassword(const char pass[6]) {
	...
	for (int i = 0; i < 5; ++i) {
		foundPair = foundPair || ((pass[i] == pass[i + 1]) && (i + 2 == 6 || pass[i] != pass[i + 2]) && (i == 0 || pass[i] != pass[i - 1]));
		if(pass[i] > pass[i+1]){
			return false;
		}
	}
	...
	for(const auto &tc: testCases) {
		...
		for(int i = 0 ; i < 6; ++i) {
			if(buffer[i] != strPass[i]) {
				std::cerr << "Bad SetPass(" << intPass << ") = " << buffer << std::endl;
				return false;
			}
		}
		...
		if(isValid != std::get<0>(tc) ){
			std::cerr << "Bad validation of " << std::get<2>(tc) << std::endl;
			return false;
		}
	}
...
int main(int,char**) {
	std::cout << "Day 4, Second" << std::endl;
